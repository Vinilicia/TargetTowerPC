...
func rotate_and_snap(degrees: float, duration := 0.3) -> void:
	...
	tween.tween_method(func(current_tweened_angle: float) -> void:
		if state.first:
			state.first = false
		...
		if use_pivot:
			...
			var current_pivot_global : Vector2 = global_position + local_pivot_pos
			
			# 3. Rotacionamos a posição global em relação a esse pivô instantâneo
			var diff : Vector2 = global_position - current_pivot_global
